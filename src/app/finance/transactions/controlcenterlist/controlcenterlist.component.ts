...
export class ControlcenterlistComponent implements OnInit {
  ...
  public arCC: Array<HIHFinance.ControllingCenter> = [];
  public listData: HIHFinance.DocumentItemWithBalance[] = [];
  public currentCCName: string;
  ...
  loadCCList(): void {
    if (environment.DebugLogging) {
      console.log("Entering loadCCList of FinanceAccountList");
    }
    ...
    let apiUrl = environment.ApiUrl + "/api/financecontrollingcenter";
    ...
    this._http.get(apiUrl, { headers: headers })
      ...
      .subscribe(data => {
        if (data instanceof Array) {
          ...
          if (this.arCC.length > 0) {
            this.showDocItems(this.arCC[0]);
          }
        }
      },
      error => {
  ...
  showDocItems(ccobj: HIHFinance.ControllingCenter) {
    if (environment.DebugLogging) {
    ...
    params.set('ccid', ccobj.Id.toString());
    this.currentCCName = ccobj.Name;
