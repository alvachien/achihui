...
describe('OrganizationDetailComponent', () => {
  ...
  describe('duplicate pre-check', () => {
    ...
    const mkOrg = (id: number, nn: string, cn = ''): Organization => {
    ...
    it('blocks create when a cached organization has the same NativeName', async () => {
      fetchAllOrganizationsSpy.and.returnValue(of([mkOrg(5, 'Test org')]));
    ...
    it('blocks create when the input NativeName hits a cached ChineseName (cross-match)', async () => {
      fetchAllOrganizationsSpy.and.returnValue(of([mkOrg(5, 'Other', 'Test org')]));
    ...
    it('allows create when ChineseNames are both empty and NativeNames differ', async () => {
      fetchAllOrganizationsSpy.and.returnValue(of([mkOrg(5, 'Other', '')]));
    ...
    it('still submits when the pre-check fetch fails (API guard is authoritative)', async () => {
      fetchAllOrganizationsSpy.and.returnValue(asyncError<string>('Service failed'));
    ...
    it('edit mode: saving the record with its own name unchanged is not blocked', async () => {
      ...
      const self = mkOrg(122, 'Test org');
      storageService.fetchAllOrganizationTypes.and.returnValue(asyncData([]));
      readOrganizationSpy.and.returnValue(asyncData(self));
      updateOrganizationSpy.and.returnValue(asyncData(self));
      // The home list contains the edited record itself - self-exclusion must kick in.
      fetchAllOrganizationsSpy.and.returnValue(of([self]));
