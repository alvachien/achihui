...
describe("OperatorFilterPipe", () => {
  ...
  it("3. Test string", () => {
    const arrst: UIDisplayString[] = pipe.transform(
      arUIString,
      GeneralFilterValueType.string
    );
  ...
  it("4. Test boolean", () => {
    const arrst: UIDisplayString[] = pipe.transform(
      arUIString,
      GeneralFilterValueType.boolean
    );
    expect(arrst.length).toBeGreaterThan(0);
