  ...
  describe('fetchAllCollections', () => {
    ...
    it('should return expected currencies (called once)', () => {
      ...
      service.fetchAllCollections().subscribe({
        next: (curries: any) => {
          expect(curries.length).withContext('should return expected currencies').toEqual(fakeData.blogCollectionAPI.length);
          expect(service.Collections.length).withContext('should have buffered').toEqual(fakeData.blogCollectionAPI.length);
        },
        error: (fail: any) => {
          // Empty
        }
