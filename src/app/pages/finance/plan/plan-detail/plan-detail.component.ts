...
export class PlanDetailComponent implements OnInit, OnDestroy {
  ...
  get saveButtonEnabled(): boolean {
    if (this.isFieldChangable) {
      if (this.detailFormGroup.valid) {
        const planType = this.detailFormGroup.get("typeControl")
          ?.value as PlanTypeEnum;
        switch (planType) {
  ...
  constructor(
    ...
    this.arFinPlanTypes =
      UIDisplayStringUtil.getFinancePlanTypeEnumDisplayStrings();
    ...
    this.detailFormGroup = new UntypedFormGroup(
      {
        ...
        currControl: new UntypedFormControl(
          this.homeService.ChosedHome!.BaseCurrency,
          [Validators.required]
        ),
      },
      [dateRangeValidator]
            ...
            .subscribe(
              (rsts: any) => {
                ...
                this.detailFormGroup
                  ...
                  ?.setValue(planObj.StartDate!.toDate());
                this.detailFormGroup
                  .get("endDateControl")
                  ?.setValue(planObj.TargetDate!.toDate());
                this.detailFormGroup
                  .get("despControl")
                  ?.setValue(planObj.Description);
                this.detailFormGroup
                  .get("accountControl")
                  ?.setValue(planObj.AccountID);
                this.detailFormGroup
                  .get("acntCtgyControl")
                  ?.setValue(planObj.AccountCategoryID);
                this.detailFormGroup
                  .get("tranTypeControl")
                  ?.setValue(planObj.TranTypeID);
                this.detailFormGroup
                  .get("controlCenterControl")
                  ?.setValue(planObj.ControlCenterID);
                this.detailFormGroup
                  .get("amountControl")
                  ?.setValue(planObj.TargetBalance);
                this.detailFormGroup
                  .get("currControl")
                  ?.setValue(planObj.TranCurrency);
                this.detailFormGroup
                  .get("typeControl")
                  ?.setValue(planObj.PlanType);
  ...
  private _generatePlan(): Plan {
    ...
    dataInstance.StartDate = moment(
      this.detailFormGroup.get("startDateControl")?.value as Date
    );
    dataInstance.TargetDate = moment(
      this.detailFormGroup.get("endDateControl")?.value as Date
    );
    dataInstance.Description = this.detailFormGroup.get("despControl")?.value;
    dataInstance.PlanType = this.detailFormGroup.get("typeControl")
      ?.value as PlanTypeEnum;
    switch (dataInstance.PlanType) {
      case PlanTypeEnum.AccountCategory:
        dataInstance.AccountCategoryID =
          this.detailFormGroup.get("acntCtgyControl")?.value;
        break;

      case PlanTypeEnum.Account:
        dataInstance.AccountID =
          this.detailFormGroup.get("accountControl")?.value;
        break;

      case PlanTypeEnum.ControlCenter:
        dataInstance.ControlCenterID = this.detailFormGroup.get(
          "controlCenterControl"
        )?.value;
        break;

      case PlanTypeEnum.TranType:
        dataInstance.TranTypeID =
          this.detailFormGroup.get("tranTypeControl")?.value;
        break;

      default:
        break;
    }
    dataInstance.TargetBalance =
      this.detailFormGroup.get("amountControl")?.value;
